...
import noImage from '../../assets/png/placeholders/320.png'
  ...
  return (
    <div className={modalStyles.modal}>
      <div className={styles.content}>
        <div className={styles.preview} style={{ backgroundImage: `url(${transparentImage})` }}>
          <img src={image.full || image.url} alt="" className={styles.image} />
        </div>
        <div className={styles.info}>
          <h1 className={styles.title}>{image.title}</h1>
          <div className={styles.profile}>
            <div className={styles.profileImg} style={{ backgroundImage: `url(${image.creator_thumb || noImage})` }} />
            {external ? (
              <p>
                Photo by{' '}
                <a className={styles.name} href={`${image.creator_link}?utm_source=Gwaphics&utm_medium=referral`}>
                  {image.creator}
                </a>
                {' on '}
                <a className={styles.name} href="https://unsplash.com/?utm_source=Gwaphics&utm_medium=referral">
                  Unsplash
                </a>
              </p>
            ) : (
              <p>
                By <span className={styles.name}>{user.username}</span>
              </p>
            )}
          </div>
          {external ? (
